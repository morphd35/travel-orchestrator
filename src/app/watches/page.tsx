...
export default function WatchesPage() {
  ...
  // Load watches from unified Edge system
  const loadWatches = async () => {
    try {
      ...
      const watches = Array.isArray(data) ? data : (data.watches || []);
      setWatches(watches.map((watch: any) => ({ ...watch, source: 'edge' as const })));
      setError(null);
    } catch (err: any) {
  ...
  // Load watches on component mount
  useEffect(() => {
    loadWatches();
  }, []);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold text-slate-900">My Watches</h1>
                <p className="text-xs text-slate-500">Price monitoring dashboard</p>
              </div>
            </div>
